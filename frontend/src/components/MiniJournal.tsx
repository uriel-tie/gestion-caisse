        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="px-4 py-3 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                <h3 className="font-bold text-gray-700 text-sm flex items-center">
                    <Clock size={16} className="mr-2 text-gray-400"/> {t('components.mini_journal.title')}
                </h3>
            </div>
            <div className="divide-y divide-gray-100">
                {operations.length === 0 ? (
                    <p className="p-4 text-center text-sm text-gray-400">{t('components.mini_journal.empty')}</p>
                ) : (
                    operations.map((op: any) => (
                        <div key={op.id} className="p-3 flex justify-between items-center hover:bg-gray-50 transition">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="text-sm font-medium text-gray-800">{op.motif || t('components.mini_journal.default_reason')}</p>
